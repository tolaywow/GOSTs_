...
std::vector<uint8_t> Hash512::give_hash(uint8_t *text, uint64_t length)
{
	uint8_t zero[0x40] = {0};
	uint64_t step = 0;
	// if |M|>=512
	...
	N_(static_cast<uint16_t>(length));
...
void Hash512::g(uint8_t *text, uint8_t *definition)
{
	uint8_t var1[0x40] = {0};
	uint8_t var2[0x40] = {0};
...
void Hash512::xors(uint8_t *key, uint8_t *block)
{
	for (size_t i = 0; i < 0x40; i++)
	{
		block[i] ^= key[i];
...
void Hash512::P(uint8_t *block)
{
	uint8_t var_block[0x40] = {0};
...
void Hash512::L(uint8_t *block)
{
	for (size_t i = 0; i < 0x8; i++)
	{
		l((uint64_t *)(block + 0x8 * i));
	}
}

void Hash512::l(uint64_t *block)
{
...
void Hash512::Key_create(uint8_t *block)
{
	for (size_t i = 0; i < 0x40; i++)
	{
		Keys[0][i] = block[i];
...
void Hash512::N_(uint16_t length)
{
	uint8_t var[0x40] = {0};

	var[0x0] = static_cast<uint8_t>(length);
	var[0x1] = static_cast<uint8_t>(length >> 0x8);

	summ_512(N, var);
}

void Hash512::summ(uint8_t *block)
{
	summ_512(Summ, _text);
}

void Hash512::summ(uint16_t length)
{
	uint8_t var[0x40] = {0};
...
void Hash512::summ_512(uint8_t *block, uint8_t *summator)
{
	uint8_t var = 0;
	for (size_t i = 0; i < 0x40; i++)
	{
		if (int16_t(block[i]) + int16_t(summator[i] + var) > uint8_t(block[i] + summator[i]) + var)
		{
			uint8_t var1 = (int16_t(block[i]) + summator[i] + var) >> 0x8;
...
void Hash512::E(uint8_t *K, uint8_t *m)
{
	Key_create(K);
